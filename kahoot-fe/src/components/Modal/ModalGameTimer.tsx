...
const ModalGameTimer = ({
  ...
  const [openModalShowNotify, setOpenModalShowNotify] =
    ...
    (async () => {
      try {
        ...
        if (response.statusCode !== HttpStatusCode.Ok) {
          console.error(
            "Error fetching scheduled room data:",
            response.message
          );
          return;
        }
  ...
  return (
    ...
    >
      <div className="text-white text-sm font-coiny">
        ...
        <div className="mt-10">
          <p className="text-lg font-coiny">Setup time for the game</p>
        ...
        <div className="flex flex-col">
          <div className="mt-4">
            <p className="text-lg font-coiny">Choose channels to notify</p>
            <MultiSelectDropdown
              dropdownPosition="bottom"
              selectedValues={
                selectedChannels?.map((item) => item.channelId) || []
              }
              options={
                appState.channelList?.map((item) => ({
              ...
              onSelect={(selectedOptions: OptionType[]) => {
                const selectedChannels = appState.channelList?.filter((item) =>
                  selectedOptions.some(
                    (option) => option.value === item.channelId
                  )
                );
          ...
          <div className="mt-4 flex items-center gap-2">
            ...
            <label
              htmlFor="send-notify-checkbox"
              className="text-md font-coiny text-slate-300 peer-checked:text-slate-200 cursor-pointer select-none"
            >
        ...
        <div className="mt-3">
          <div className="flex justify-between">
            <p className="text-lg font-coiny">Notify message</p>
            <img
              onClick={() => setOpenModalShowNotify(true)}
              alt="icon-eye"
          ...
          <RichTextBox
            ...
            rows={7}
            className="rounded-md border-[2px] w-full min-h-[200px] text-justify text-sm"
          ></RichTextBox>
        </div>
        <div className="flex justify-center gap-3 mt-4">
          <Button
            onClick={onClose}
            className="text-center bg-[#ded525] font-coiny "
          >
            {cancelText}
          </Button>
          <Button
        ...
        >
          <div className="mt-3">
            ...
            <div className="mt-3 border-t-2 pt-2">
              <p className="font-coiny text-justify">
                Scan the QR code or enter the code below to join the game:
              </p>
              <img
                alt="icon-qr"
                src="/icons/icon-qr-code.png"
                className="w-[180px]"
              />
            </div>
          </div>
