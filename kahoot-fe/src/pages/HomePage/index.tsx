    ...
    <div className="w-full">
      ...
      >
        ...
        <div className="flex flex-col gap-3 min-w-[200px] sm:min-w-[250px] md:min-w-[300px]">
          <Button
            onClick={handleGoToSearchGame}
            className="font-diablo text-xl sm:text-2xl md:text-3xl bg-[#6BB3E0]"
          >
            {/* Tìm Game */}
            Find Game
          </Button>
          <Button
            ...
            className="font-diablo text-xl sm:text-2xl md:text-3xl bg-[#6F7CDD]"
          >
            {/* Game của tôi */}
            My Games
          </Button>
          <Button className="font-diablo text-xl sm:text-2xl md:text-3xl bg-[#B2ADFF]">
            {/* Cài Đặt */}
            Settings
          </Button>
        </div>
        <div className="max-w-[250px] pl-2 fixed top-10 md:top-5 right-5 w-full bg-[#3b3d3978] cursor-pointer rounded-full flex items-center gap-2 shadow-xl filter brightness-100 hover:brightness-110 transition-all active:brightness-100">
          ...
          <div className="w-[50px] h-[50px] rounded-full border-2 border-white">
            <div
              ...
              style={{
                backgroundImage: `url(${
                  authState.currentUser?.avatar || "/icons/icon-user.png"
                })`,
              }}
            ></div>
          </div>
      ...
      {appState.isShowSplash && (
        <div
          className="fadeOut"
          style={{ animationDelay: appState.isShowSplash ? "8s" : "unset" }}
        >
          <div
