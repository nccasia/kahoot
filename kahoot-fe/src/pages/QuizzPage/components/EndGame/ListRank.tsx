  ...
  return (
    <div className='w-full max-w-[800px] mt-3'>
      {userRankings?.map((userRanking, index) => (
        <div
          key={index}
          className='flex font-diablo w-full bg-[#00020dad] rounded-sm p-2 mb-2 min-h-[50px] items-center cursor-pointer filter hover:brightness-125 hover:bg-[#00020d]'
        >
          ...
          <div className='flex-1'>
            <div className='w-full bg-[#ff0000c7] h-[25px] rounded-md text-start relative overflow-hidden'>
              ...
              ></div>
              <span className='absolute left-2 text-white top-1/2 -translate-y-1/2'>
                {userRanking.totalCorrect}/{totalQuestion}
              </span>
            </div>
          </div>
          <div className='w-[50px]'>
