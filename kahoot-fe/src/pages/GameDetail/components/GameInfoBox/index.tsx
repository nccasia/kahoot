...
const GameInfoBox = ({ gameInfo, totalQuestion, owner }: GameInfoBoxProps) => {
  ...
  const [openModalUpdateGameTimer, setOpenModalUpdateGameTimer] =
    useState(false);
  ...
  const handleUpdateTimer = async (
    scheduleData: ICreateScheduleRoom,
    roomId?: string
  ) => {
    if (!roomId) {
      toast.error(
    ...
    const response = await roomServices.updateScheduledRoom(
      roomId,
      scheduleData
    );
    if (response.statusCode !== HttpStatusCode.Ok) {
      console.log("error", response);
      toast.error(
        // "Cập nhật thời gian không thành công"
      ...
      <div className="flex flex-col gap-3 mt-2 justify-center items-center">
        ...
        <div className="flex gap-3 w-full">
          <Button
            ...
            className="text-center bg-[#aa37ca] font-coiny text-lg w-full"
          >
            {/* Chơi thử */}
            Simulate Game
          </Button>
      ...
      {gameState.listRooms?.length > 0 && (
        <div className="mt-6">
          <div className="flex justify-between items-center">
            <span className="mr-2 font-coiny">
              {/* Lịch sử phòng chơi */}
              Game Room History:
            </span>
            <Button
          ...
          <div className="mt-2 flex flex-col ">
            <div className="w-full">
              {gameState.listRooms.map((room) => (
                <div
                  className="bg-[#466CF7A1] rounded-xl flex justify-between gap-2 items-center mt-3 p-3"
                  key={room.id}
                >
                  <div className="flex flex-col items-start gap-[2px]">
                    ...
                    >
                      {/* Phòng: */}
                      Room:{" "}
                      <span className="text-[#ffee8df6]">{room.code}</span>
                    </span>
                    <span
                      className={`font-coiny text-sm ${
                        RoomStatusColor[
                          room?.status as keyof typeof RoomStatusColor
                        ]
                      }`}
                    >
                  ...
                  <div className="flex gap-1 items-center">
                    ...
                    {room?.status === ERoomStatus.Finished && (
                      <Button className="!bg-transparent !p-1 !min-w-0 !shadow-none !rounded-none hover:!translate-x-0 hover:!translate-y-0 hover:!shadow-none active:!shadow-none">
                        <img
