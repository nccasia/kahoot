...
const QuestionContent = ({
  ...
  const handleDeleteAnswer = (index: number) => {
    if (question.answerOptions.options.length <= 2)
      return toast.warning(
        // "Tối thiểu 2 đáp án cho mỗi câu hỏi lựa chọn"
        "At least 2 answers for each choice question"
      );
    const newQuestion = {
      ...question,
      answerOptions: {
    ...
    if (
      ...
      newQuestion.answerOptions.correctIndex >=
        newQuestion.answerOptions.options.length
    ) {
  ...
  const handleAddAnswer = () => {
    if (question.answerOptions.options.length >= 4)
      return toast.warning(
        // "Tối đa 4 đáp án cho mỗi câu hỏi lựa chọn"
        "Maximum of 4 answers for each choice question"
      );
    const newQuestion = {
      ...question,
      answerOptions: {
  ...
  return (
    <div className="body sm:p-2 md:p-4 font-coiny text-white">
      ...
      <div className="flex items-center flex-wrap">
        <span className="inline-block sm:w-[150px] md:w-[200px] w-full text-start text-xl md:text-2xl">
          Câu hỏi:
        </span>
      ...
      <div className="flex flex-col gap-3 mt-2 pt-2 border-t-2 border-gray-100">
        {question.mode === EQuestionTypes.TEXT ? (
          <div className="flex items-center flex-wrap gap-1">
            <span className="inline-block min-w-[195px] text-start text-xl md:text-2xl">
              {/* Đáp án : */}
              Answer:
            </span>
          ...
          <>
            {question.answerOptions?.options.map((option, index) => (
              <div key={index} className="flex items-center flex-wrap gap-1">
                <span className="inline-block min-w-[150px] md:min-w-[200px] text-start text-xl md:text-2xl">
                  {/* Đáp án {index + 1}: */}
                  Answer {index + 1}:
                </span>
                    ...
                    {question.mode === EQuestionTypes.SINGLE_CHOICE ? (
                      ...
                      >
                        <div className="absolute left-0 top-0 z-10 w-full h-full flex items-center justify-center">
                          ...
                          >
                            {question.answerOptions.correctIndexes?.includes(
                              index
                            ) && (
                              <span className="w-2 h-2 bg-white rounded-full block blur-[1px]"></span>
                            )}
        ...
        {question.answerOptions.options.length <= 3 ? (
          <div className="ml-0 md:ml-[208px] flex gap-2">
            <Button
              ...
              className="bg-[#6B00E7] rounded-md min-w-[50px]"
            >
      ...
      <Collapse
        hasError={
          question.questionStatus !== EQuestionErrorTypes.NO_ERROR
            ? questionErrorTypes[question.questionStatus]
            : undefined
        }
